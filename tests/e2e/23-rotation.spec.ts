 ...
 *
 * The geometry is pinned in tests/unit/storefront-rotation.test.ts. What can
 * only be proven here is the WIRING, and specifically the two things a tilted
 * tile breaks if the gesture code is wrong: that dragging a tilted tile still
 * tracks the cursor in SCREEN space (the drag offset must land in `translate`,
 * outside the rotation, or the tile flies off at an angle), and that grabbing
 * a tilted tile's edge still resizes from the edge under the hand rather than
 * from wherever its bounding box happens to be.
 */
...
test.describe("tilting a block", () => {
  ...
  test("grabbing a turned tile's edge resizes the side under the hand", async ({
    ...
    // Edge grabbing works on the box the seller SEES, in screen space. Pull
    // the right-hand side of a turned tile to the right and it gets wider on
    // screen, exactly as it would if the tile were level. Reading the drag in
    // the tile's own turned space instead is what used to send it growing off
    // at an angle to the hand.
  ...
  test("a half-turned tile grows the way the hand pulls, not the other way", async ({
    ...
    // The inversion the local-space version had: at half a turn the tile's own
    // south edge is the one at the TOP of the screen, so dragging the top edge
    // upwards grew the block downwards.
