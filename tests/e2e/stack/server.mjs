...
async function loadUser(id) {
  const { rows } = await pool.query(
    `select id, email, encrypted_password, raw_user_meta_data, raw_app_meta_data, created_at, updated_at from auth.users where id = $1`,
    [id],
  );
  return rows[0] ?? null;
}
...
async function userJson(row) {
  ...
  return {
    ...
    phone: "",
    // A spec can make an account look Google-signed-in by writing
    // raw_app_meta_data.providers directly, as GoTrue itself records it.
    app_metadata: Array.isArray(row.raw_app_meta_data?.providers)
      ? row.raw_app_meta_data
      : { provider: "email", providers: ["email"] },
    user_metadata: row.raw_user_meta_data ?? {},
    identities: (Array.isArray(row.raw_app_meta_data?.providers)
      ? row.raw_app_meta_data.providers
      : ["email"]
    ).map((provider) => ({ provider, id: row.id, user_id: row.id, identity_id: row.id })),
...
// Shaped exactly like real GoTrue's qr_code (checked against production on
// 2026-09-24): SVGo output, which opens with an XML declaration AND a
// comment. A bare <svg> here once hid a parser that refused every real QR
// code. No `data:` prefix: GoTrue sends the SVG itself and auth-js adds
// `data:image/svg+xml;utf-8,` on the way to the app, so a prefix here would
// arrive doubled.
...
async function handleAuth(req, res, url) {
  ...
  if (req.method === "PUT" && path === "/user") {
    ...
    if (!auth) return;
    const body = await readBody(req);
    // GoTrue's rule, verified against the production project on 2026-09-24
    // with a throwaway account: with a verified factor on the account, an
    // aal1 session may change neither the password nor the email
    // (insufficient_aal). The app never relies on it (its own gate refuses
    // such a session first), but the mock should not be laxer than the real
    // thing.
...
const next = spawn(
  ...
  [join(REPO, "node_modules", "next", "dist", "bin", "next"), "dev", "-p", String(NEXT_PORT)],
  {
    ...
    env: {
      ...
      // Passkeys: a fixed test sealing key (32 zero bytes, never a real one),
      // and this server's own origin, since NEXT_PUBLIC_APP_URL (from
      // .env.local) names the developer's :3000. The RP ID is still
