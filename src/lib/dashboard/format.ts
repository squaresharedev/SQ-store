import type { Locale } from "@/i18n/locales";
import type { Currency } from "@/types/product";
import { formatCents } from "@/lib/format/money";
import { dateTimeFormat, intlTag } from "@/lib/format/intl";
import type { MoneyByCurrency } from "./queries";

// Display helpers for dashboard money/dates. Money is ALWAYS integer cents in
// (from the DB) and formatted major units out — never float math on amounts.
// The single cents→display formatter lives in lib/format/money.ts (client-safe);
// this module re-exports it for existing dashboard imports.

export { formatCents } from "@/lib/format/money";

/**
 * Per-currency totals joined for display, e.g. `"€64.00 · US$12.00"`.
 * Returns null when there is nothing to show (caller renders its zero state).
 */
export function formatMoney(money: MoneyByCurrency, locale: Locale): string | null {
  const parts = (Object.entries(money) as [Currency, number][])
    // EUR (primary market) first, stable order after.
    .sort(([a], [b]) => (a === b ? 0 : a === "EUR" ? -1 : 1))
    .map(([currency, cents]) => formatCents(cents, currency, locale));
  return parts.length > 0 ? parts.join(" · ") : null;
}

/** Short order date, e.g. "2 Jul". Server-rendered only (no hydration risk). */
export function formatOrderDate(isoDate: string, locale: Locale): string {
  const date = new Date(isoDate);
  return Number.isNaN(date.getTime())
    ? "—"
    : dateTimeFormat(intlTag(locale, "en-IE"), { day: "numeric", month: "short" }).format(date);
}
